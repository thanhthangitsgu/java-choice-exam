 ...
 * @author ASUS
 */
public class PanelQuanLyMonHoc {
    ...
    public PanelQuanLyMonHoc(String pnCenter) {
        ...
        pnPage.add(btNext);
        //rieng
        MonBUS mh = new MonBUS();
        String[] temp = {"", "Mã môn học", "Tên môn", "Số tín chỉ"};
        info = temp;
        MyLabel lbMaMH = new MyLabel("Mã môn học :");
        lbMaMH.setPreferredSize(new Dimension(120, 30));
        top.add(lbMaMH);
        ...
        MyLabel lbSTC = new MyLabel(info[3]);
        MyTextField txtSTC = new MyTextField();
        top.add(lbSTC);
        top.add(txtSTC);
        ...
        tba.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (i != -1) {
                    ...
                    txtSTC.setText(String.valueOf(model.getValueAt(i, 3)));
                }
            }
        });
        ...
        bts[1].addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                if (txtTen.getText().equals("") || txtSTC.getText().equals("")) {
                    new ErrorFrame("Vui lòng điền đầy đủ thông tin!");
                } else if (!CheckError.isNameClass(txtTen.getText())) {
                    new ErrorFrame("Tên Môn Không Hợp Lệ");
                } else if (!CheckError.isInteger(txtSTC.getText())) {
                    new ErrorFrame("Số Tín Chỉ Phải Là Số Nguyên");
                } else {
                    ErrorFrame err = new ErrorFrame("Bạn có chắc chắn muốn thêm không?");
                    err.btnOK.addMouseListener(new MouseAdapter() {
                        public void mouseClicked(MouseEvent arg0) {
                            MonDTO dtMon = new MonDTO();
                            dtMon.setTenMon(txtTen.getText());
                            dtMon.setSoTinChi(Integer.parseInt(txtSTC.getText()));
                            mh.them(dtMon);
                    ...
                    if (txtTen.getText().equals("") || txtSTC.getText().equals("")) {
                        ...
                        err.btnOK.addMouseListener(new MouseAdapter() {
                            public void mouseClicked(MouseEvent arg0) {
                                ...
                                dtMon.setTenMon(txtTen.getText());
                                dtMon.setSoTinChi(Integer.parseInt(txtSTC.getText()));
                                mhhoantac = mh.getObject(String.valueOf(model.getValueAt(i, 1)));
                                mh.sua(dtMon, String.valueOf(model.getValueAt(i, 1)));
        ...
        bts[3].addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                if (check == 1) {
                    mh.khoiphuc(mahoantac);
                } else if (check == 2) {
                    mh.xoavinhvien(mahoantac);
                } else if (check == 3) {
                    mh.sua(mhhoantac, mahoantac);
                } else if (check == 4) {
                    for (int i = 0; i < checkbox.size(); i++) {
                        mh.khoiphuc(checkbox.get(i));
                    }
                }

                vtData.clear();
                vtData.addAll(mh.getThongTin(Integer.parseInt(txtPage.getText())));
                model.setDataVector(vtData, vtHeader);
                tba.setModel(model);
                guiBase.isEnabled(bts[3]);
                check = 0;
        ...
        btImportExcel.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                mh.importExcel(file.getFileExcelNameOpen(pnPaint));
            }
        });

        btExportExcel.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent arg0) {
                mh.exportExcel(file.getFileExcelNameSave(pnPaint));
            }
        });
